...
Updated on 2026-04-18 after the first implementation batch.
...
- Android `versionName` 已推进到 `0.4.11`
- 当前状态适合作为第一批 Android-only 优化结果提交
...
CodexRemote Android 端应停在“远程控制台”而不是“移动 IDE”。
...
- `apps/android/app/src/main/java/dev/codexremote/android/navigation/Screen.kt`
...
- 这更适合当前 CodexRemote，成本低、价值高、不会把手机端做重
...
- `apps/android/app/src/main/java/dev/codexremote/android/ui/sessions/SessionVoiceInputController.kt`
- `apps/android/app/src/main/java/dev/codexremote/android/ui/sessions/SessionDetailScreen.kt`
...
- `apps/android/app/src/main/java/dev/codexremote/android/ui/sessions/ComposerSuggestions.kt`
- `apps/android/app/src/main/java/dev/codexremote/android/ui/sessions/SessionDetailScreen.kt`
...
本阶段不追求完整 Markdown 引擎，而是做一个轻量、够用、适合手机的表格渲染 MVP：

- 优先识别标准 markdown table
- 列数较少时按横向可滚动表格显示
- 超窄屏或超多列时允许降级为“按行展开的结构化卡片”
...
- `apps/android/app/src/main/java/dev/codexremote/android/ui/sessions/RichTextBlocks.kt`
...
- `apps/android/app/src/test/java/dev/codexremote/android/ui/sessions/*`
...
- `v0.4.16`：删除“精准控制台 / 项目导航 / 项目按更新时间分组”这类说明性文字，只保留标签层表达
...
- 桌面端常见 markdown 表格在 Android 端不再丢失列结构

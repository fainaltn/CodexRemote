# findeck 对标 Paseo 改进计划书

## 文档目标

本计划书用于回答两个问题：

1. 相比 `getpaseo/paseo`，findeck 当前最明显的差距是什么
...
findeck 当前的问题，不是“功能明显不够”，而是“重路径过多，壳层持续性不足，状态表达还不够产品化”。
...
项目已经有明确的 `Console` 方向，但目前这套语言更多落在颜色、卡片、文案上，还没有完全覆盖：
...
如果这轮计划执行完成，用户对 findeck 的主观感受应当从：

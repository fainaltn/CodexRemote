---
name: apk-smb-sync
description: Build a simple post-update Android APK delivery flow that reads the app version and copies a dated CodexRemote APK into the LAN SMB share. Use when the user mentions APK update sync, Android release handoff, SMB share upload, 局域网共享, or asks to archive the latest APK by version date.
---
...
- The default label must be `CodexRemote`; never reuse names from unrelated projects or SMB share history.
...
- The default artifact name is `YYYY-MM-DD_CodexRemote_v<version>.apk`.
...
- If a generated filename or target path contains a project name unrelated to `CodexRemote`, treat that as a skill bug and correct the skill/scripts before publishing.
